...
-- ARQUIVO: docs/supabase-initial-schema.sql
-- BANCO DE DADOS: PostgreSQL / Supabase
-- VERSÃO: 4.4 (Validação Rigorosa de Roles e Preparação para Execução)
-- ==============================================================================
-- ATENÇÃO: Este script é idempotente e seguro para execução manual no Supabase SQL Editor.
-- Nenhuma credencial privada, chave secreta ou token está contido neste arquivo.
-- ==============================================================================
...
-- Atribuir todas as permissões ao Admin e Síndico
...
-- Atribuir permissões consultivas ao Conselho Fiscal
insert into public.role_permissions (role_id, permission_id)
...
-- Atribuir permissões básicas ao Morador
insert into public.role_permissions (role_id, permission_id)
...
-- ------------------------------------------------------------------------------
-- 17. TRIGGER DE AUTOCADASTRO SEGURO (ROLE MORADOR PADRÃO, SEM ELEVAÇÃO)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 18. TRIGGER DE PROTEÇÃO CONTRA PRIVILEGE ESCALATION E CROSS-TENANT NO PROFILES
-- ------------------------------------------------------------------------------
...
begin
  ...
  -- Se o usuário atual está alterando o próprio registro e não é admin
  ...
  -- Admin só pode gerenciar perfis pertencentes ao seu próprio condomínio
...
-- ------------------------------------------------------------------------------
-- 19. ATIVAÇÃO DE ROW LEVEL SECURITY (RLS) EM TODAS AS 14 TABELAS
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 20. POLÍTICAS DE SEGURANÇA (RLS POLICIES) ESTBITAMENTE ISOLADAS POR TENANT
-- ------------------------------------------------------------------------------

-- [ROLES & PERMISSIONS] - Leitura para usuários autenticados, escrita bloqueada
...
-- [CONDOMINIUMS] - Somente o próprio condomínio é visível e gerenciável
...
-- [PROFILES] - Usuário vê seu perfil e membros do mesmo condomínio
...
-- [UNITS] - Leitura restrita ao condomínio, mutação restrita a admins locais
...
-- [UNIT OWNERS] - Visualização de donos da unidade ou gestão administrativa local
...
-- [UNIT RESIDENTS] - Visualização de moradores da unidade ou gestão administrativa local
...
-- [FINANCIAL ENTRIES] - Isolamento absoluto: Morador vê apenas débitos da sua unidade
...
-- [MAINTENANCE REQUESTS] - Morador abre chamado e vê os seus/áreas comuns; Admin gerencia
...
-- [ANNOUNCEMENTS] - Moradores vêem comunicados publicados do seu condomínio
...
-- [DOCUMENTS] - Controle por nível de visibilidade (all, council, admin_only)
...
-- [ASSEMBLIES] - Leitura por membros do condomínio, gestão por admins locais
...
-- [ACTIVITY LOGS] - Trilha de auditoria restrita a admins locais, inserção controlada
...
-- ------------------------------------------------------------------------------
-- Convenção de Storage: O primeiro segmento do caminho deve ser o ID do condomínio:
-- Exemplo: {condominium_id}/documento.pdf ou {condominium_id}/manutencao/foto.jpg
...
-- 1. Leitura de Documentos do Condomínio
...
-- 2. Leitura de Anexos de Manutenção do Condomínio
...
-- 3. Leitura de Atas de Assembleia do Condomínio
...
-- 4. Upload de Documentos por Administradores do Condomínio
...
-- 5. Upload de Anexos de Manutenção por Membros do Condomínio
...
-- 6. Upload de Atas de Assembleia por Administradores do Condomínio
...
-- 7. Exclusão de Arquivos no Storage por Administradores do Condomínio
